...
def lane_area_drawn(undist, warped_img, Minv, line_class):
    ...
    y_max = np.max(ploty)
    left_lane_bottom = line_class.best_fit_left[0]*y_max**2 + line_class.best_fit_left[1]*y_max + line_class.best_fit_left[2]
    right_lane_bottom = line_class.best_fit_right[0]*y_max**2 + line_class.best_fit_right[1]*y_max + line_class.best_fit_right[2]
    midpoint_bird = int((left_lane_bottom + right_lane_bottom)/2)
...
def image_marking(result, Minv, midpoint_bird, curvature):
    ...
    if midpoint_raw <= midpoint:
        dif_dir = 'left'
    else:
        dif_dir = 'right'
    dif_center = np.round(abs(midpoint_raw - midpoint)*xm_per_pix, decimals = 2)
    
    font = cv2.FONT_HERSHEY_SIMPLEX
    text_left = 'Left Radius of Curvature = ' + str(int(curvature[0])) + '(m)'
    text_right = 'Right Radius of Curvature = ' + str(int(curvature[1])) + '(m)'
    text_dif = 'Vehicle is ' + str(dif_center) + 'm ' + dif_dir + ' of center'
    cv2.putText(result,text_left,(20,60), font, 2,(255,255,255),4,cv2.LINE_AA)
    cv2.putText(result,text_right,(20,140), font, 2,(255,255,255),4,cv2.LINE_AA)
    cv2.putText(result,text_dif,(20,220), font, 2,(255,255,255),4,cv2.LINE_AA)
...
out = cv2.VideoWriter('output_videos/project_video_lane_drawn_usingprior_sanity_mark.mp4', 
...
#        print(counter)
        ...
        curvature = measure_curvature_meters(warped, lane_det)
